...
impl Interceptor for InterceptorObj {
    ...
    fn intercept<'a>(&'a self, request: &'a mut Request<Body>, cx: Context<'a>) -> InterceptorFuture<'a, Self::Err> {
...
trait DynInterceptor: Send + Sync {
    fn dyn_intercept<'a>(&'a self, request: &'a mut Request<Body>, cx: Context<'a>) -> InterceptorFuture<'a, Box<dyn Error>>;
}
